...
class M_admin extends CI_Model
{
    ...
    public function produksiPerTanggal($date, $plant_id)
    {
        ...
        AND prd.produksi_id = spt.admisecsgp_mstproduction_produksi_id  and spt.status = 1  ORDER BY spt.nama_shift ASC");
        return $query;
    }
    ...
    {
        $q = $this->db->query("SELECT MAX(RIGHT(id_jadwal_patroli,4)) AS kd_max FROM admisecsgp_trans_jadwal_patroli WHERE DATE(date_patroli)=CURDATE()");
        $kd = "";
        if ($q->num_rows() > 0) {
