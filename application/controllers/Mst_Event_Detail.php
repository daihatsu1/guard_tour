...
class Mst_Event_Detail extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view("mst_event_detail", $data);
    ...
    public function form_add(Type $var = null)
    {
        ...
        $data = [
            ...
            'event'         => $this->M_patrol->ambilData("admisecsgp_mstevent", ['status' => 1]),
        ];
        // $this->template->load("template/template", "add_mst_event_Detail", $data);
        $this->load->view("template/sidebar", $data);
        $this->load->view("add_mst_event_detail", $data);
        $this->load->view("template/footer");
    }
    ...
    public function input()
    {
        ...
        $input = $this->M_patrol->mulitple_upload("admisecsgp_msteventdtls", $data);

        if ($input) {
            $this->session->set_flashdata('info', '<i class="icon fas fa-check"></i> Berhasil input data');
            redirect('Mst_Event_Detail');
        } else {
    ...
    {
        $where = ['id' => $id];
        $del = $this->M_patrol->delete("admisecsgp_msteventdtls", $where);
        if ($del) {
    ...
    public function edit()
    {
        ...
        $data = [
            ...
            'plant'      => $this->M_patrol->ambilData("admisecsgp_mstplant", ['status' => 1]),
            'zona'       => $this->M_patrol->ambilData("admisecsgp_mstzone", ['status' => 1, 'admisecsgp_mstplant_id' => $plant_id]),
            'check'      => $this->M_patrol->ambilData("admisecsgp_mstckp", ['status' => 1, 'admisecsgp_mstzone_id' => $zona_id]),
            'objek'      => $this->M_patrol->ambilData("admisecsgp_mstobj", ['status' => 1, 'admisecsgp_mstckp_id' => $check_id]),
            'event'      => $this->M_patrol->ambilData("admisecsgp_mstevent", ['status' => 1]),
            'zona_id'    => $zona_id,
            'plant_id'   => $plant_id,
            'check_id'   => $check_id
        ...
        $this->load->view("template/sidebar", $data);
        $this->load->view("edit_mst_event_detail", $data);
        $this->load->view("template/footer");
    }
    ...
    public function update()
    {
        ...
        $where = ['id' => $id];
        $update = $this->M_patrol->update("admisecsgp_msteventdtls", $data, $where);
        if ($update) {
    ...
    public function form_upload_event_detail()
    {
        ...
        $this->load->view("template/sidebar", $data);
        $this->load->view("upload_mst_event_detail", $data);
        $this->load->view("template/footer");
    }
    ...
    public function upload_event()
    {
        ...
        if ($upload) {
            ...
            redirect('Mst_Event_Detail/form_upload_event_detail');
        }
    }
    ...
    public function multipleDelete()
    {
        ...
        // var_dump($id_event);
        $delete = $this->M_patrol->multiple_delete("admisecsgp_msteventdtls", $id_event);
