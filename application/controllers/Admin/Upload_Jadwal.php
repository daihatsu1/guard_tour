...
class Upload_Jadwal extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['plant_master']  = $this->M_admin->ambilData("admisecsgp_mstplant", ['status' => 1, 'admisecsgp_mstsite_id' => $id_wil_user]);

        // $this->template->load("template/template", "upload_jadwal", $data);
        $this->load->view("template/admin/sidebar", $data);
        $this->load->view("admin/upload_jadwal", $data);
    ...
    public function upload(Type $var = null)
    {
        ...
        $save_production = $this->M_admin->mulitple_upload("admisecsgp_mstproductiondtls", $data_prod);
        $save_patroli    =  $this->M_admin->mulitple_upload("admisecsgp_mstjadwalpatroli", $data_jadwal);

        if ($save_patroli && $save_production) {
            $this->session->set_flashdata('info', '<i class="icon fas fa-check"></i> Berhasil upload data');
            redirect('Admin/Upload_Jadwal');
