...
class Mst_Jadwal_Patroli extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if (isset($_POST['lihat'])) {
            ...
            $data['user']         = $this->M_admin->headerjadwalPatroli($plant_id, $tahun . '-' . convert_bulan($bulan));
        }
        $data['link']          = $this->uri->segment(2);
        $data['plant']         = $this->M_admin->ambilData("admisecsgp_mstplant", ['status' => 1, 'admisecsgp_mstsite_site_id' => $id_wil_user]);
        // $this->template->load("template/template", "mst_jadwal", $data);
        $this->load->view("template/admin/sidebar", $data);
        $this->load->view("admin/mst_jadwal_patroli", $data);
