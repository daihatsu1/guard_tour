...
class Login extends CI_Controller
{
    ...
    public function cekLogin()
    {
        ...
        $cekUser = $this->db->query("SELECT usr.id ,   usr.npk , usr.name , ru.level  , usr.admisecsgp_mstsite_id , usr.admisecsgp_mstplant_id 
        FROM `admisecsgp_mstusr` usr  , `admisecsgp_mstroleusr`  ru
        WHERE ru.id = usr.admisecsgp_mstroleusr_id and usr.password = '" . $password . "' and usr.name = '" . $name . "' ");
        ...
        if ($cekUser->num_rows() > 0) {
            ...
            switch ($data->level) {
                case 'SUPERADMIN':
                    $this->session->set_userdata("id_token", $data->id);
                    $this->session->set_userdata("npk", $data->npk);
                    $this->session->set_userdata("name", $data->name);
                    $this->session->set_userdata("role", $data->level);
                    $this->session->set_userdata("site_id", $data->admisecsgp_mstsite_id);
                    $this->session->set_userdata("plant_id", $data->admisecsgp_mstplant_id);
                    redirect('Dashboard');
                    break;
                case 'ADMIN':
                    $this->session->set_userdata("id_token", $data->id);
